...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      load();
      this._render();
      // Defensive re-assert: a host that streams/mounts attributes (src,
      // id) can finish landing the real value on this element without it
      // ever reaching us as a fresh attributeChangedCallback/_render() —
      // observed on streamed DCs, where a slot can sit "rendered but not
      // filled" (empty ring stuck over what should be a loaded photo)
      // long after its src attribute already reads correctly. _render()
      // is idempotent, so a short poll across the first few seconds
      // costs nothing once things have already settled and reliably
      // clears the stuck case otherwise.
      this._settleTimers = [50, 200, 600, 1500, 3000].map((ms) =>
        setTimeout(() => { if (this.isConnected) this._render(); }, ms));
    }
    ...
    _render() {
      ...
      if (url && !attrError) {
        ...
        // A host that streams/mounts attributes can render this element
        // with a blank src for one glitch frame before the real value
        // lands (no user action involved). Clearing immediately would tear
        // down an already-filled slot for that one frame and get stuck
        // (nothing re-fills it once the real src re-arrives with no visual
        // change to react to). Defer one microtask and re-check: a newer
        // render (fill or clear) bumps _renderGen and this stale pass bails,
        // leaving that newer render's own outcome in place.
        const gen = this._renderGen;
        this._clearTries = (this._clearTries || 0) + 1;
        const giveUp = this._clearTries > 4;
        Promise.resolve().then(() => {
          if (gen !== this._renderGen) return; // a newer render already resolved this
          // Live re-check, not just a re-render wait: poll the actual
          // attribute/store directly in case this host's next update never
          // reaches us as a fresh _render() call.
          const liveStored = this.id ? getSlot(this.id) : this._local;
          const liveUrl = (liveStored && liveStored.u) || this.getAttribute('src') || '';
          if (liveUrl && !giveUp) { this._render(); return; }
          this._clearTries = 0;
          this.removeAttribute('data-swapping');
